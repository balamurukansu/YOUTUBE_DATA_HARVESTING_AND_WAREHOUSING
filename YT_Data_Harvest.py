...
cursor.execute("CREATE DATABASE IF NOT EXISTS YT_Scrap")

# Switch to 'mydatabase'
cursor.execute("USE YT_Scrap")
...
def data():
    ...
    if questions == '1. What are the names of all the videos and their corresponding channels?':
        ...
        cursor.execute("""select a.channel_name AS Channel_Name, a.video_name,Total_Comments AS Video_Title, b.Total_Comments from yt_videodetails a left join (SELECT video_id,COUNT(Comment_Id) AS Total_Comments FROM yt_commentdetails GROUP BY video_id) b on a.video_id=b.video_id ORDER BY b.Total_Comments DESC""")
        df = pd.DataFrame(cursor.fetchall(),columns=cursor.column_names)
        st.table(df)
